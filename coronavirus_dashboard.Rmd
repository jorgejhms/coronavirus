...
g_diario <- function(k, n) {
   ggplot(k, aes(x = fecha, y = n)) +
      ...
      geom_line(aes(y = rollmean(n, 7, fill = NA)), size = 1.2) +
...
```{r}
contagios_total <- last(minsa_db$positivos_cum)
valueBox(prettyNum(contagios_total, big.mark = ","),icon = "fa-user", color = "warning")
```

### Personas fallecidas
```{r}
fallecidos_total <- last(minsa_db$fallecidos_cum)
valueBox(prettyNum(fallecidos_total, big.mark = ","),icon = "fa-cross", color = "danger")
```

### Nuevos contagios en el último día
```{r}
contagios_dia <- last(minsa_db$positivos)
valueBox(prettyNum(contagios_dia, big.mark = ","), color = "warning")
```

### Fallecidos en el último día
```{r}
fallecidos_dia <- last (minsa_db$fallecidos)
...
tabla_mapa <- cbind(tabla_mapa, st_coordinates(st_centroid(tabla_mapa))) #Crea posiciones medias X, Y a partir de las figuras de cada departamento
